...
/**
 * An AVL Search Tree Implementation Null Sentinels used to avoid null code case
 * 
...
public class AVL<T extends Comparable<? super T>> {
	...
	/**
	 * Recursively insert a node into the tree, update heights and balance on the
	 * way back up
	 * 
	 ...
	 */
	private AVLNode insert(T data, AVLNode root) throws IllegalArgumentException {
		if (root == null)
			throw new IllegalArgumentException("Root cannot be null.");
		if (root == nullSentinel)
			throw new IllegalArgumentException("Root cannot be nullSentinel.");
		if (data == null)
		...
		if (root.data.compareTo(data) > 0) { // new data is less than
			...
			// TODO potential rotations during insert requires a different case here than
			// BST
	...
	/**
	 * Helper method to calculate appropriate height for a node Assumes both
	 * children have already corrected their heights
	 * 
	...
	private AVLNode rotateCW(AVLNode oldP) {
		AVLNode oldC = oldP.left;
		oldP.left = oldC.right;
		oldC.right = oldP;
		updateHeight(oldP);
		updateHeight(oldC);
		return oldC;
	...
	private AVLNode rotateCCW(AVLNode oldP) {
		AVLNode oldC = oldP.right;
		oldP.right = oldC.left;
		oldC.left = oldP;
		updateHeight(oldP);
		updateHeight(oldC);
		return oldC;
	...
	private AVLNode doubleRotateFromLeft(AVLNode oldP) {
		oldP.left = rotateCCW(oldP.left);
		return rotateCW(oldP);
	...
	private AVLNode doubleRotateFromRight(AVLNode oldP) {
		oldP.right = rotateCW(oldP.right);
		return rotateCCW(oldP);
	}
	 ...
	 * 
	 * @return a String consisting of all elements concatenated together in post
	 *         order
	 */
	 ...
	 * Public facing interface for recursive pre-order traversal printing
	 * 
	 * @return a String consisting of all elements concatenated together in pre
	 *         order
	 */
	 ...
	 * 
	 * @return a String consisting of all elements concatenated together in level
	 *         order
	 */
	public String printLevelOrder() {
		...
		while (!q.isEmpty()) {
			AVLNode cur = q.poll();
			if (cur.left != nullSentinel)
				q.add(cur.left);
			if (cur.right != nullSentinel)
				q.add(cur.right);
			result += cur.data + " ";
		}
		return result;
